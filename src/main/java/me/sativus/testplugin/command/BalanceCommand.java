                                ...
                                .then(Commands.argument("amount", DoubleArgumentType.doubleArg())
                                        .executes(ctx -> {
                                            ...
                                            if (walletManager.withdraw(sender, amount)) {
                                                ...
                                                sender.sendRichMessage("You have sent <green>" + amount
                                                        + "</green> to <green>" + target.getName() + "</green>.");
                                                target.sendRichMessage("You have received <green>" + amount
                                                        + "</green> from <green>" + sender.getName() + "</green>.");
                                            } else {
                                                sender.sendRichMessage("You do not have enough balance to send <red>"
                                                        + amount + "</red>.");
                                            }
                                ...
                                .then(Commands.argument("amount", DoubleArgumentType.doubleArg())
                                        .executes(ctx -> {
                                            ...
                                            target.sendRichMessage(
                                                    "Your balance has been set to: <green>" + amount + "</green>");
                                            ctx.getSource().getSender().sendMessage(
                                                    "Set balance of " + target.getName() + " to " + amount);
                                            return Command.SINGLE_SUCCESS;
                                        }))))
                        ...
                        .then(Commands.argument("target", ArgumentTypes.player())
                                .executes(ctx -> {
                                    ...
                                    if (target.getUniqueId().equals(sender.getUniqueId())) {
                                        ...
                                        if (ctx.getSource().getSender().hasPermission("testplugin.balance.get.other")) {
                                            double balance = walletManager.getBalance(target);
                                            ctx.getSource().getSender()
                                                    .sendMessage(target.getName() + "'s balance is: " + balance);
                                            return Command.SINGLE_SUCCESS;
                                        } else {
                                            sender.sendMessage(
